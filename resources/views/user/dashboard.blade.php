        ...
        <aside class="w-64 bg-white shadow-lg min-h-screen">
            <div class="p-6">
                ...
                <nav class="space-y-2">
                    ...
                    <a href="#panduan" onclick="showSection('panduan')" 
                       class="nav-link flex items-center p-3 text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all duration-200">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.468 9.587 5.097 8.323 5.097c-.31 0-.612.15-.746.25L6.5 6.44v6.44a1.5 1.5 0 001.5 1.5c.31 0 .612-.15.746-.25L12 12.44v1.81z"></path>
                        </svg>
                        Panduan
                    </a>
            ...
            <div class="absolute bottom-6 left-6 right-6">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full flex items-center p-3 text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors duration-200">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <main class="flex-1 p-8">
            ...
            <header class="flex justify-between items-center mb-8">
                ...
                <div class="flex items-center space-x-4">
                    <span class="px-3 py-1 bg-indigo-100 text-indigo-800 rounded-full text-sm">{{ Auth::user()->role }}</span>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-700">
                                <strong>Menunggu Billing:</strong> {{ $pendingBilling->count() }} pengajuan sedang diproses billing oleh admin.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-blue-700">
                                <strong>Pembayaran Tersedia:</strong> {{ $pendingPayment->count() }} billing siap untuk dibayar. Silakan download dan upload bukti pembayaran.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="bg-yellow-50 p-4 rounded-lg">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <p class="text-2xl font-bold text-yellow-900">{{ $submissions->where('status', 'Menunggu Verifikasi')->count() }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-green-50 p-4 rounded-lg">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <p class="text-2xl font-bold text-green-900">{{ $submissions->whereIn('status', ['Diterima', 'Berhasil'])->count() }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-red-50 p-4 rounded-lg">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <p class="text-2xl font-bold text-red-900">{{ $submissions->where('status', 'Ditolak')->count() }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                                                ...
                                                @if ($submission->status === 'Diterima' && $submission->pembayaran)
                                                    @if ($submission->pembayaran->billing_file_path)
                                                        <a href="{{ route('user.pembayaran.download', $submission->pembayaran->id) }}" 
                                                           class="text-indigo-600 hover:text-indigo-900">Download Billing</a>
                                                        
                                                        @if (!$submission->pembayaran->payment_proof_path)
                                                            <a href="{{ route('user.pembayaran.upload', $submission->id) }}" 
                                                               class="text-green-600 hover:text-green-900">Upload Bukti</a>
                                                        @endif
                                                    @endif
                                                @elseif ($submission->status === 'Berhasil')
                                                    <a href="{{ asset('storage/' . $submission->cover_letter_path) }}" target="_blank" 
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                                            Belum ada pengajuan. <a href="#pengajuan" onclick="showSection('pengajuan')" class="text-indigo-600 hover:text-indigo-900">Buat pengajuan baru</a>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="text-center">
                                <div class="bg-white p-4 rounded border border-blue-200">
                                    ...
                                    <a href="#" class="text-blue-600 hover:text-blue-800 text-sm">Download .docx</a>
                                </div>
                            </div>
                            <div class="text-center">
                                <div class="bg-white p-4 rounded border border-blue-200">
                                    <svg class="w-8 h-8 text-blue-600 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.468 9.587 5.097 8.323 5.097c-.31 0-.612.15-.746.25L6.5 6.44v6.44a1.5 1.5 0 001.5 1.5c.31 0 .612-.15.746-.25L12 12.44v1.81z"/>
                                    </svg>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex space-x-4">
                            <button type="button" id="btn-pnbp" class="px-6 py-2 rounded-lg bg-indigo-600 text-white font-semibold shadow transition-colors duration-200 hover:bg-indigo-700">
                                PNBP (Berbayar)
                            </button>
                            <button type="button" id="btn-nonpnbp" class="px-6 py-2 rounded-lg bg-gray-200 text-gray-700 font-semibold shadow transition-colors duration-200 hover:bg-gray-300">
                                Non-PNBP (Gratis)
                            </button>
                        </div>
                    </div>
                    ...
                    <!-- Form Pengajuan -->
                    <form action="{{ route('user.submit') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        <input type="hidden" name="kategori" id="kategori_input" value="PNBP">

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label for="jenis_data" class="block text-sm font-medium text-gray-700 mb-2">Jenis Data yang Diajukan</label>
                                <select id="jenis_data" name="jenis_data" required class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            <div>
                                <label for="keperluan" class="block text-sm font-medium text-gray-700 mb-2">Keperluan Penggunaan Data</label>
                                <textarea id="keperluan" name="keperluan" rows="3" required class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" placeholder="Jelaskan keperluan penggunaan data..."></textarea>
                            </div>

                            <div>
                                <label for="tanggal_mulai" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Mulai</label>
                                <input type="date" id="tanggal_mulai" name="tanggal_mulai" required class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                            </div>

                            <div>
                                <label for="tanggal_selesai" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Selesai</label>
                                <input type="date" id="tanggal_selesai" name="tanggal_selesai" required class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                            </div>
                        </div>
                        ...
                        <div id="file-upload-pnbp" class="space-y-4">
                            <div>
                                <label for="file_surat_pnbp" class="block text-sm font-medium text-gray-700 mb-2">Upload Surat Pengantar *</label>
                                <input type="file" id="file_surat_pnbp" name="files[]" accept=".pdf,.doc,.docx" required class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        <div id="file-upload-nonpnbp" class="hidden space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Upload Surat Pengantar dari Instansi *</label>
                                <input type="file" name="files[]" accept=".pdf,.doc,.docx" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                                <p class="text-sm text-gray-500 mt-1">Format: PDF, DOC, DOCX (Max: 5MB)</p>
                            </div>
                            
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Upload Dokumen Proposal/Karya Ilmiah</label>
                                <input type="file" name="files[]" accept=".pdf,.doc,.docx" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                                <p class="text-sm text-gray-500 mt-1">Format: PDF, DOC, DOCX (Max: 5MB)</p>
                            </div>
                            
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Upload Dokumen Pendukung Lainnya (Opsional)</label>
                                <input type="file" name="files[]" accept=".pdf,.doc,.docx" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                                <p class="text-sm text-gray-500 mt-1">Format: PDF, DOC, DOCX (Max: 5MB)</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end space-x-4">
                            <button type="reset" class="px-6 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors duration-200">
                                Reset Form
                            </button>
                            <button type="submit" class="px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors duration-200 font-semibold">
                                Ajukan Surat
                            </button>
                        </div>
                    </form>
            ...
            <section id="panduan" class="content-section hidden">
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <p class="text-gray-600 mb-6">Klik pada jenis data di bawah ini untuk melihat detail, contoh, dan syarat pengajuannya.</p>

                    <div class="space-y-4">
                        @forelse ($guidelines as $guideline)
                            <div class="border border-gray-200 rounded-lg overflow-hidden">
                                <button class="accordion-header w-full flex justify-between items-center p-4 bg-gray-50 hover:bg-gray-100 transition-colors duration-200" onclick="toggleAccordion(this)">
                                    ...
                                    @if ($guideline->example_data && is_array($guideline->example_data))
                                        ...
                                        <div class="bg-gray-50 p-4 rounded-lg">
                                            @foreach ($guideline->example_data as $key => $value)
                                                <p class="text-sm text-gray-600"><strong>{{ ucfirst($key) }}:</strong> {{ $value }}</p>
                                            @endforeach
                                        </div>
                                    @endif
                                </div>
    ...
    <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        ...
        <form id="editForm" class="space-y-4" method="POST" enctype="multipart/form-data">
            ...
            <div>
                ...
                <input type="text" id="edit_no_surat" class="w-full p-3 border border-gray-300 rounded-lg bg-gray-100" readonly>
            </div>
            
            <div>
                <label for="edit_jenis_data" class="block text-sm font-medium text-gray-700 mb-2">Jenis Data</label>
                <select id="edit_jenis_data" name="jenis_data" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                    @if($guidelines && $guidelines->count() > 0)
                        @foreach ($guidelines as $guideline)
                            <option value="{{ $guideline->title }}">{{ $guideline->title }}</option>
                        @endforeach
            ...
            <div>
                <label for="edit_tanggal_mulai" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Mulai</label>
                <input type="date" id="edit_tanggal_mulai" name="tanggal_mulai" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
            </div>
            
            <div>
                <label for="edit_tanggal_selesai" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Selesai</label>
                <input type="date" id="edit_tanggal_selesai" name="tanggal_selesai" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
            </div>
            
            <div>
                <label for="edit_keperluan" class="block text-sm font-medium text-gray-700 mb-2">Keperluan</label>
                <textarea id="edit_keperluan" name="keperluan" rows="3" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"></textarea>
            </div>
            
            <div>
                <label for="edit_surat_pengantar" class="block text-sm font-medium text-gray-700 mb-2">Upload Ulang Surat Pengantar</label>
                <input type="file" id="edit_surat_pengantar" name="surat_pengantar" accept=".pdf,.doc,.docx" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (pnbpBtn && nonpnbpBtn) {
        pnbpBtn.addEventListener('click', function() {
            ...
            fileUploadPNBP.querySelectorAll('input').forEach(input => input.removeAttribute('disabled'));
            fileUploadNonPNBP.querySelectorAll('input').forEach(input => input.setAttribute('disabled', 'disabled'));
        });
        ...
        nonpnbpBtn.addEventListener('click', function() {
            ...
            // Enable/disable inputs
            fileUploadPNBP.querySelectorAll('input').forEach(input => input.setAttribute('disabled', 'disabled'));
            fileUploadNonPNBP.querySelectorAll('input').forEach(input => input.removeAttribute('disabled'));
        });
    }
    ...
    window.showEditModal = function(submissionData) {
        ...
        // Fill form
        document.getElementById('edit_no_surat').value = submissionData.submission_number || 'SUB-' + submissionData.id;
